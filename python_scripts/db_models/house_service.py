...
@asynccontextmanager
async def get_repositories(session: Optional[AsyncSession] = None) -> AsyncGenerator[Dict[str, Union[GalleryHouseRepository, DetailHouseRepository, FloorPlanRepository]], None]:
...
class HouseService:
    ...
    async def store_gallery_houses(self, gallery_houses: List[GalleryHouse]) -> List[DBGalleryHouse]:
        ...
        async with get_repositories(self.session) as repos:
            for house in gallery_houses:
                try:
                    ...
                    existing_house: Optional[DBGalleryHouse] = await repos['gallery'].get_by_address(house.address, house.city)
                    ...
                    if existing_house:
                        ...
                        skipped_count += 1
                        logger.info(f"House already exists: {house.address}, {house.city}, using ID: {existing_house.id}")
                        continue
                    ...
                    logger.info(f"Stored gallery house: {stored_house.address}, {stored_house.city}")
    ...
    async def store_detail_houses(self, detail_houses: List[DetailHouse]) -> List[DBDetailHouse]:
    ...
    async def store_detail_house(self, detail_house: DetailHouse, gallery_id: Optional[int] = None) -> Optional[DBDetailHouse]:
        """Store a detail house and return the detail house object"""
        async with get_repositories(self.session) as repos:
            try:
                if gallery_id is None and detail_house.gallery_id is not None:
                    gallery_id = detail_house.gallery_id
                elif gallery_id is None:
                    logger.warning(f"No gallery ID provided for detail house: {detail_house}")
                    return None

                # Check if house already exists
                existing_house: Optional[DBDetailHouse] = await repos['detail'].get_by_address(
                    detail_house.address,
                    detail_house.postal_code, 
                    detail_house.city
                )
                ...
                if existing_house:
                    ...
                    if existing_house.has_changes(detail_house):
                        detail_house_obj = await repos['detail'].update(existing_house.id, detail_house)
                        logger.info(f"Updated detail house with changes: {detail_house_obj.address}, {detail_house_obj.city}")
                    else:
                        logger.info(f"No changes needed for detail house: {existing_house.address}, {existing_house.city}")
                        detail_house_obj = existing_house
                else:
                    # Store new house in database
                    detail_house_obj = await repos['detail'].create(detail_house, gallery_id)
                    logger.info(f"Stored new detail house: {detail_house_obj.address}, {detail_house_obj.city}")
    ...
    async def store_crawler_results(self, gallery_data: List[GalleryHouse], detail_data: Optional[DetailHouse] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if detail_data:
                ...
                gallery_id = result['gallery_houses'][-1].id if result['gallery_houses'] else None
